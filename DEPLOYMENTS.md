...
|------|-------|---------|------------|-------------------|-------|
| 2026-06-29 | 6.1 | **mainnet** | `0x71ca3a5ca48174b91c215e30e42d4ca019da1a85e15e0b74a59979f6a8e2dc5b` | `0x14c6ce9f17daec0d358b01becc22aeff722123634cddb88d911b8c40f98c37cb` | **MAINNET H1 redeploy.** Same H1 source as the testnet canonical (recipient binding), linked vs mainnet DeepBook `0x0e735f…`. Published via SDK (gas from address balance) + `sui keytool sign` (key never left keystore), publish tx `vLKqhXHk…`; UpgradeCap `0x128863979a323c89b56b9fdff856899ce7e512d6f4664b9ea5e467ccb50faa93`. Mainnet registry `0x5d65372d87b8e9c1d356d13073b65bdde913a02efd546032705ff1c8dcbb1247` + mandate `0x277843cd…` created (`6dPT4Bt1…`). The wrong-recipient `seal_approve` smoke was not run on mainnet (node-fetch RPC flakiness from the build env); H1 is proven on **testnet** (`E_RECIPIENT`, `A2dWJNk7…`) on byte-identical source + 34 unit tests. Supersedes `0xd37ca38e`. |
| 2026-06-29 | 6.1 | testnet | `0x98164c30fe76cbfec0f822083d1a4e37ffba49af9c2257e3b9d3f2f04b74a7f1` | `0x14c6ce9f17daec0d358b01becc22aeff722123634cddb88d911b8c40f98c37cb` | **H1 recipient binding (audit fix).** `policy::check` now requires `intent.recipient == mandate.owner`, so a leaked one-shot witness cannot redirect a policy-passing trade to an attacker. **34 Move tests** (2 new: `evaluate_wrong_recipient`, `authorize_wrong_recipient_aborts` → `E_RECIPIENT` code 13). New app registry `0x0bdb186a4f3bc18b6a54689c437f57cd4d481694b64daf60d2d871f77858468f` (DEEP_SUI allowlisted); UpgradeCap `0x8fefc2cbae399e41b8eacb5f70eb6daad17444bc87e8c8a9140daf85254236f3`. Publish tx `7Lo4Z6jULWUzjUVotagMMWuxcjzgWWMCRWUrf9Q45fLa`. **Supersedes `0x7a7ee718` as the testnet canonical; mainnet redeploy of H1 is pending.** |
| 2026-06-28 | 6 | **mainnet** | `0xd37ca38e54a3218bbdf7417b9817d0075ebd56ed65584a382af87854e2605066` | `0x14c6ce9f17daec0d358b01becc22aeff722123634cddb88d911b8c40f98c37cb` | **MAINNET deploy.** Same source as the testnet canonical, linked against mainnet DeepBook (`0x0e735f…`, lineage `0x2c8d60…`). Publisher SUI arrived in Sui's **address-balance** accumulator (exchange `send_funds`), which CLI 1.74 cannot select as a gas coin - so published via SDK (gas paid from address balance) signed with `sui keytool sign` (**key never left the keystore**). Publish tx `JCKDTvQo…`. **Mainnet smoke:** `new_mandate`+registry created; compliant `seal_approve` **success** (`2z8CxQ6C…`); over-cap `seal_approve` **aborts `E_OVER_CAP`** (`6JWnSZKX…`, `MoveAbort policy::check, 1`). Stretch `nautilus/` enclave-verifier package built + **4 tests green** (ed25519, not published). Tests now: 34 sentinel + 4 nautilus + 6 sdk = 44 (mainnet predates the H1 recipient test). |
...
| Object | ID | Stage |
|--------|----|-------|
| **MAINNET package (current, H1)** | `0x71ca3a5ca48174b91c215e30e42d4ca019da1a85e15e0b74a59979f6a8e2dc5b` | 6.1 |
| MAINNET UpgradeCap (current) | `0x128863979a323c89b56b9fdff856899ce7e512d6f4664b9ea5e467ccb50faa93` | 6.1 |
| MAINNET registry (current) | `0x5d65372d87b8e9c1d356d13073b65bdde913a02efd546032705ff1c8dcbb1247` | 6.1 |
| MAINNET package (prior, pre-H1; over-cap/seal smoke `6JWnSZKX`/`2z8CxQ6C`) | `0xd37ca38e54a3218bbdf7417b9817d0075ebd56ed65584a382af87854e2605066` | 6 |
| MAINNET UpgradeCap (prior) | `0xace01e916483451b51ef0ee74474ae4e4e20d44c627cb708b0fede1113bcc856` | 6 |
...
| **Package (current testnet, H1)** | `0x98164c30fe76cbfec0f822083d1a4e37ffba49af9c2257e3b9d3f2f04b74a7f1` | 6.1 |
| UpgradeCap (current testnet) | `0x8fefc2cbae399e41b8eacb5f70eb6daad17444bc87e8c8a9140daf85254236f3` | 6.1 |
| **App market registry (current, DEEP_SUI allowlisted; arming uses this)** | `0x0bdb186a4f3bc18b6a54689c437f57cd4d481694b64daf60d2d871f77858468f` | 6.1 |
| **Live app (Vercel)** | https://sentinel-clay-web.vercel.app/ | 5 |
| App market registry (prior) | `0x8b49d0d7afde529a8784f3f255b1fa2168519988aae242f5bf3a881b6a7f7c1f` | 5 |
| Package (prior - Seal adapter) | `0x7a7ee7186ccb69b2b250e7b08fc31b8ccfadae9a7596a352112f7aa3e72a77f9` | 4 |
| UpgradeCap (prior) | `0x0250e15d0b4d32f859473813d31d3069062bd9a4e719240ff7aefc1b0d047cdf` | 4 |
